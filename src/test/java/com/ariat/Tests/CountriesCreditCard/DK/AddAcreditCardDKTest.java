...
/**
 * Tests for Add Credit card Denmark
 * @author aila.bogasieru@ariat.com
 *
 */


public class AddAcreditCardDKTest extends BaseTest{
	...
	public void addCreditCardDKTest() {
		...
		logger.info("Starting add a credit card Denmark test");
		...
		logger.info("Finishing add a credit card Denmark test");
  } 
	
	@Test(priority = 1)
	public void addCreditCardFromPaymentInfoUKTest() {
		logger.info("Starting add credit card from Payment Info Denmark test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
		homePageDK = (HomePageDK) homePage.chooseEULocation(euCountry.DK, euCountry.DK.getCurrencyISO());
		...
		logger.info("Finishing add credit card from Payment info Denmark test");
