package com.ariat.Tests.MyAccountCountries.CA;
...
public class MyAccountMiddleNavigationCATest extends BaseTest {
	...
	@Test(priority = 3)
	public void myAccountAccessPaymentInfoCATest() {
		logger.info("Starting payment info Canada test...");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageCA = (HomePageCA) homePage.chooseGlobalLocation(country.CA, country.CA.getCurrencyISO());
		signInPage = homePageCA.returnSignInPage();
		...
		myAccountPage.myAccountClick();
		myAccountPage.logoutMiddle();
		logger.info("Finishing payment info Canada test...");
	}
