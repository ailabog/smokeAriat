package com.ariat.Tests.CountriesDeleteAddress.CA;
...
public class DeleteAddressCATest extends BaseTest {
	...
	@Test(priority = 0)
	public void deleteAddressCATest() {
		logger.info("Starting deleting address Canada test");
		...
		logger.info("Finishing deleting address Canada test");
	}

	@Test(priority = 1)
	public void deleteAddressFromEditCATest() {
		logger.info("Starting deleting address from Edit address Canada test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageCA = (HomePageCA) homePage.chooseGlobalLocation(country.CA, country.CA.getCurrencyISO());
		signInPage = homePageCA.returnSignInPage();
		...
		logger.info("Finishing deleting address from Edit address Canada test");
	}
