...
/**
 * Tests for Add Credit card Italy
 * @author aila.bogasieru@ariat.com
 *
 */


public class AddCreditCardITTest extends BaseTest{
	...
	public void addCreditCardDKTest() {
		...
		logger.info("Finishing add a credit card Italy test");
  } 
	
	@Test(priority = 1)
	public void addCreditCardFromPaymentInfoITTest() {
		logger.info("Starting add credit card from Payment Info Italy test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
		homePageIT = (HomePageIT) homePage.chooseEULocation(euCountry.IT, euCountry.IT.getCurrencyISO());
		...
		logger.info("Finishing add credit card from Payment info Italy test");
