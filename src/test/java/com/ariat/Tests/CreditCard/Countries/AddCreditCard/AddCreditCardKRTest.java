...
/**
 * Tests for Add Credit card Korea
 * @author aila.bogasieru@ariat.com
 *
 */


public class AddCreditCardKRTest extends BaseTest{
	...
	public void addCreditCardJETest() {
		...
		logger.info("Starting add a credit card Korea test");
		...
		logger.info("Finishing add a credit card Korea test");
  } 
	
	@Test(priority = 1)
	public void addCreditCardFromPaymentInfoJETest() {
		logger.info("Starting add credit card from Payment Info Korea test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageKR = (HomePageKR) homePage.chooseGlobalLocation(country.KR, country.KR.getCurrencyISO());
		signInPage = homePageKR.returnSignInPage();
		...
		logger.info("Finishing add credit card from Payment info Korea test");
