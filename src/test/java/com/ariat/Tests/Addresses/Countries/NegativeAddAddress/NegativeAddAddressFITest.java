...
/**
 * Adds negative address and checks the address was created for Finland
 * 
 * @author aila.bogasieru@ariat.com
 *
 */

public class NegativeAddAddressFITest extends BaseTest {
	...
	@Test(priority=0)
	public void negativeAddAddressFITest() {
		logger.info("Starting add negative address FI test");
	...
	@Test(priority=1)
	public void negativeAddAddressFITestWildcards() {
		logger.info("Starting add negative address FI test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
	...
	@Test(priority=2)
	public void negativeAddAddressFITestMissingValues() {
		logger.info("Starting add negative address FI test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageUK = (HomePageUK) homePage.chooseEULocation(euCountry.UK, euCountry.UK.getCurrencyISO());
