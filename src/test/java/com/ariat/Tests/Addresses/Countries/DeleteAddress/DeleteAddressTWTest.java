...
public class DeleteAddressTWTest extends BaseTest {
	...
	public void deleteAddressJPTest() {
		logger.info("Starting deleting address Taiwan test");
		...
		logger.info("Finishing deleting address Taiwan test");
	}

	@Test(priority = 1)
	public void deleteAddressFromEditJPTest() {
		logger.info("Starting deleting address from Edit address Taiwan test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageTW = (HomePageTW) homePage.chooseGlobalLocation(country.TW, country.TW.getCurrencyISO());
		signInPage = homePageTW.returnSignInPage();
		...
		logger.info("Finishing deleting address from Edit address Taiwan test");
	}
