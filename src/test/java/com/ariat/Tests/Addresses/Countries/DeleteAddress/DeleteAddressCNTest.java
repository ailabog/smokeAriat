...
public class DeleteAddressCNTest extends BaseTest {
	...
	@Test(priority = 0)
	public void deleteAddressCNTest() {
		logger.info("Starting deleting address China test");
		...
		logger.info("Finishing deleting address China test");
	}

	@Test(priority = 1)
	public void deleteAddressFromEditJPTest() {
		logger.info("Starting deleting address from Edit address China test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageCN = (HomePageCN) homePage.chooseGlobalLocation(country.CN, country.CN.getCurrencyISO());
		signInPage = homePageCN.returnSignInPage();
		...
		logger.info("Finishing deleting address from Edit address China test");
	}
