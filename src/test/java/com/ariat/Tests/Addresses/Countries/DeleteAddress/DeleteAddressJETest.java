...
public class DeleteAddressJETest extends BaseTest {
	...
	public void deleteAddressDKTest() {
		logger.info("Starting deleting address Jersey test");
		...
		logger.info("Finishing deleting address Jersey test");
	}

	@Test(priority = 1)
	public void deleteAddressFromEditDKTest() {
		logger.info("Starting deleting address from Edit address Jersey test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageJE = (HomePageJE) homePage.chooseGlobalLocation(country.JE, country.JE.getCurrencyISO());
		signInPage = homePageJE.returnSignInPage();
		...
		logger.info("Finishing deleting address from Edit address Jersey test");
	}
