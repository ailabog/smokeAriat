...
public class DeleteAddressTRTest extends BaseTest {
	...
	public void deleteAddressJPTest() {
		logger.info("Starting deleting address Turkey test");
		...
		logger.info("Finishing deleting address Turkey test");
	}

	@Test(priority = 1)
	public void deleteAddressFromEditJPTest() {
		logger.info("Starting deleting address from Edit address Turkey test");
		homePage = new HomePage(new ChromeDriver());
		homePage.load(environment.DEVELOPMENT.getURL());
		homePageTR = (HomePageTR) homePage.chooseGlobalLocation(country.TR, country.TR.getCurrencyISO());
		signInPage = homePageTR.returnSignInPage();
		...
		logger.info("Finishing deleting address from Edit address Turkey test");
	}
